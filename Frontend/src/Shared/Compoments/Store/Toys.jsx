...
function ToysProductsDetails() {
    ...
    useEffect(() => {
        const getToysProducts = async () => {
            try {
                setLoading(true);
                // No token check here - we want to fetch products regardless
                ...
                console.error(error);
                Swal.fire("Error", "Failed to fetch toy products", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return <div className="loading-message">Loading toy products...</div>;
    }
    ...
    <Link to="/home/store/MedicationsProductsDetails" className="store-link">Medications</Link>
        ...
        {token && (
            ...
            <div className="all-cats-container">
                <h2 className="management-dashboard-title">TOYS</h2>
...
export default ToysProductsDetails;